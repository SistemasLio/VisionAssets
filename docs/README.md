# Documentação VisionAssets

Versão navegável no site: [sistemaslio.github.io/VisionAssets](https://sistemaslio.github.io/VisionAssets/) (build com VitePress a partir desta pasta).

Índice em tabela abaixo. Convenções de IDs e fluxo de trabalho: [GOVERNANCE.md](GOVERNANCE.md).

## Negócio

| Documento | Conteúdo |
|-----------|----------|
| [business/VISION.md](business/VISION.md) | Problema, visão, stakeholders |
| [business/REQUIREMENTS.md](business/REQUIREMENTS.md) | Requisitos funcionais e não funcionais, MoSCoW |
| [business/GLOSSARY.md](business/GLOSSARY.md) | Termos e siglas |
| [business/OPEN-QUESTIONS.md](business/OPEN-QUESTIONS.md) | Decisões pendentes (antes do ADR) |

## Produto e backlog

| Documento | Conteúdo |
|-----------|----------|
| [product/ROADMAP.md](product/ROADMAP.md) | Fases e marcos |
| [product/BACKLOG-OVERVIEW.md](product/BACKLOG-OVERVIEW.md) | Épicos, PBIs e vínculos |
| [product/templates/USER-STORY.md](product/templates/USER-STORY.md) | Template de história de usuário |
| [product/stories/README.md](product/stories/README.md) | Histórias detalhadas (`US-NNN-*.md`) |

## Técnico

| Documento | Conteúdo |
|-----------|----------|
| [technical/ARCHITECTURE.md](technical/ARCHITECTURE.md) | Visão de componentes e fluxos |
| [technical/STACK.md](technical/STACK.md) | Linguagem, libs, MSI |
| [technical/DATA-MODEL.md](technical/DATA-MODEL.md) | Entidades SQLite e evolução |
| [technical/INVENTORY-COLLECTION.md](technical/INVENTORY-COLLECTION.md) | WMI, Registry, licenças |

## Rastreabilidade e decisões

| Documento | Conteúdo |
|-----------|----------|
| [traceability/TRACEABILITY-MATRIX.md](traceability/TRACEABILITY-MATRIX.md) | Requisito → épico → implementação |
| [decisions/README.md](decisions/README.md) | Índice de ADRs |
| [decisions/ADR-000-template.md](decisions/ADR-000-template.md) | Modelo de ADR |

## Changelog

| Documento | Conteúdo |
|-----------|----------|
| [overview/CHANGELOG.md](overview/CHANGELOG.md) | Histórico versionado do projeto |
| [changelog/README.md](changelog/README.md) | Como escrever entradas |
